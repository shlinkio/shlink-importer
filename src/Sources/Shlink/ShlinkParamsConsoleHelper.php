...
class ShlinkParamsConsoleHelper implements ParamsConsoleHelperInterface
{
    ...
    {
        return [
            ShlinkParams::BASE_URL => static fn () => $io->ask('What is your Shlink instance base URL?'),
            ShlinkParams::API_KEY => static fn () => $io->ask('What is your Shlink instance API key?'),
            ImportParams::IMPORT_VISITS_PARAM => static fn () => $io->confirm(
                'Do you want to import each short URL\'s visits too?',
            ),
            ImportParams::IMPORT_ORPHAN_VISITS_PARAM => static fn () => $io->confirm(
